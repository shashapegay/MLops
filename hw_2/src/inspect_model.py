...
def forward_hooks(modules: dict) -> tuple[dict, list]:
    """Навесить forward-hooks и вернуть словарь активаций вместе с handles."""
...
def device_allocated_bytes(device: torch.device) -> int:
    """Сколько памяти занято прямо сейчас на устройстве, где лежат тензоры."""
    if device.type == "cuda":
...
class PeakMemory:
    """Пиковая память за время прогона режима."""
    ...
    def __enter__(self) -> "PeakMemory":
        ...
        if self.device.type == "cuda":
            ...
            self.used = max(self.used, int(torch.cuda.memory_allocated(self.device)))
        elif self.device.type == "mps":
            self._thread = threading.Thread(target=self._poll_mps, daemon=True)
            self._thread.start()
        return self
    ...
    def __exit__(self, *exc) -> bool:
        if self.device.type == "cuda":
            ...
            self.used = max(self.used, int(torch.cuda.max_memory_allocated(self.device)))
        elif self.device.type == "mps":
...
def measure_mode_subprocess(mode: str, params: dict) -> dict:
    """Замерить один режим в отдельном процессе, чтобы peak-метрики не смешались."""
...
def memory_profile(params: dict) -> list[dict]:
    ...
    results = []
    for mode in MODES:
        runs = [measure_mode_subprocess(mode, params) for _ in range(repeats)]
...
def main() -> None:
    ...
    if args.probe:
        # check.sh читает stdout probe-режима как UTF-8. На Windows консольная
        # кодировка может быть cp1251, поэтому служебный JSON печатаем ASCII:
        # русские строки вроде "RSS процесса" уйдут как \uXXXX и не сломают decode.
